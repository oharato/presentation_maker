...
#### 8. 事前準備ファイルの活用
- 事前に用意されたスライド画像（`.png`, `.jpg`, `.jpeg`）が `input/` ディレクトリにある場合、Markdown からの画像生成をスキップします
- 事前に用意された無音動画（`.nosound.mp4`）が `input/` ディレクトリにある場合、無音動画の生成をスキップします
- 外部ツールで作成した高品質な画像やカスタム動画を利用可能
...
- **ライブプレビュー**: Markdown入力時にスライドのリアルタイムプレビューをブラウザで表示
