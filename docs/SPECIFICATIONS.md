...
**サポートされる事前準備ファイル:**
- **スライド画像**: `input/{番号}__{タイトル}.png`, `.jpg`, `.jpeg`
- **無音動画**: `input/{番号}__{タイトル}.nosound.mp4`

**動作:**
- `input/` に事前準備された画像や無音動画がある場合、それらを優先して使用します。
- スライド画像が `input/` に存在する場合、Markdown からの画像生成をスキップします。
- 無音動画が `input/` に存在する場合、無音動画の生成をスキップします。
- 存在しない場合は通常通り `output/` に生成されます。
...
### サーバー側 (CLI / ワーカー)
- **ツール**: Puppeteer + Marked
- **入力**: Markdown文字列
- **出力**: 1920x1080px のPNG画像
- **スタイル**: CSSをHTMLにインライン埋め込み。フォントは `Noto Sans CJK JP` を優先。

### ブラウザ側 (Webアプリ - ライブプレビュー)
- **ツール**: html-to-image + Marked
- **入力**: Markdown文字列 (Webアプリの入力フォームから)
- **出力**: 1280x720px のPNG画像 Blob (ブラウザメモリ内)
- **スタイル**: サーバー側のレンダリングと整合性のあるCSSを一時コンテナに適用し、視覚的な一貫性を保つ。
- **更新**: Markdown入力時にリアルタイムで再レンダリング。
- **破棄**: スライド削除時、または新しいプレビュー生成時に古いBlob URLを破棄しメモリリークを防ぐ。
- (変更なし)
