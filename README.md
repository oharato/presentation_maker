...
CLIモードでは、事前に用意された画像や動画ファイルを活用できます：

- **スライド画像**: `output/{番号}__{タイトル}.png`, `.jpg`, `.jpeg` が存在する場合、Markdownからの画像生成をスキップ
- **無音動画**: `output/{番号}__{タイトル}.nosound.mp4` が存在する場合、無音動画の生成をスキップ
